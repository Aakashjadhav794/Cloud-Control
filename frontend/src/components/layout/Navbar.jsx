    ...
    <>
      ...
      <div className="w-full bg-white/70 backdrop-blur-xl border-b border-gray-200 fixed top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 lg:px-10 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-10 text-gray-600 text-sm font-medium">
            ...
            {!token && (
              <>
                <span onClick={() => scrollTo("features")} className="cursor-pointer hover:text-blue-600">
                  Features
                </span>
                <span onClick={() => scrollTo("pricing")} className="cursor-pointer hover:text-blue-600">
                  Pricing
                </span>
                <span onClick={() => scrollTo("how")} className="cursor-pointer hover:text-blue-600">
                  Docs
                </span>
                <span onClick={() => go("/login")} className="cursor-pointer hover:text-blue-600">
                  Login
                </span>
                <button
                  onClick={() => go("/register")}
                  className="ml-4 bg-blue-600 text-white px-5 py-2 rounded-full"
                >
                  Get Started
                </button>
              </>
            ...
            {token && (
              <>
                <span onClick={() => go("/dashboard")} className="cursor-pointer hover:text-blue-600">
                  Dashboard
                </span>
                <span onClick={() => go("/clouds")} className="cursor-pointer hover:text-blue-600">
                  Clouds
                </span>
      ...
      >
        <div className="p-6 space-y-6 text-gray-700">
          ...
          {token && (
            <>
              <div onClick={() => go("/dashboard")} className="cursor-pointer">Dashboard</div>
              <div onClick={() => go("/clouds")} className="cursor-pointer">Clouds</div>
              <div onClick={logout} className="cursor-pointer text-red-600">Logout</div>
            </>
          )}
        </div>
      </div>
